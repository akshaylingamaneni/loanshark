...
import { getTrackedMarketCaps } from "@/lib/reimbursements/market-caps";
...
function defaultTargetDay(): Date {
  const d = new Date();
...
async function upsertMarketMetadata(
...
async function getStoredPosition(borrowerAddress: string, marketKey: string) {
  return db.query.borrowerPositions.findFirst({
    where: and(
      eq(borrowerPositions.borrowerAddress, borrowerAddress),
      eq(borrowerPositions.marketKey, marketKey)
    ),
  });
}
...
function resolvePriceUsd(
  ...
  const marketPrice = (market.loanAsset as { priceUsd?: number | null })?.priceUsd;
...
async function processBorrower(params: {
  ...
  let reimbursementCreated = 0;

  if (result.reimbursement > 0 && accrual) {
    reimbursementCreated = 1;
    await db
...
export type RunReimbursementResult = {
  day: string;
...
export async function runReimbursementJob(
  options: { day?: Date } = {}
): Promise<RunReimbursementResult> {
  const targetDay = options.day ? normalizeDay(options.day) : defaultTargetDay();
  const dayStart = getUnix(targetDay);
  const dayEnd = dayStart + SECONDS_PER_DAY;

  const capsMap = await getTrackedMarketCaps();
  const trackedEntries = Object.entries(capsMap);

  if (trackedEntries.length === 0) {
    console.warn("No tracked markets configured. Nothing to process.");
    const isoDay = targetDay.toISOString().slice(0, 10);
    return {
      day: isoDay,
      marketsProcessed: 0,
  ...
  const chainIds = Array.from(new Set(trackedEntries.map(([, cap]) => cap.chainId)));
  const morphoMarkets = await getMarkets(chainIds, Math.max(100, trackedEntries.length + 20));
  const capLookup = new Map(trackedEntries);
  const targets = morphoMarkets.filter((market) => capLookup.has(market.uniqueKey));

  const skippedMarkets = trackedEntries
    .map(([key]) => key)
    .filter((key) => !targets.some((market) => market.uniqueKey === key));

  console.log(
    `Running reimbursement calculation for ${targetDay.toISOString().slice(0, 10)} across ${
      targets.length
    } markets`
  );
  ...
  for (const market of targets) {
    const cap = capLookup.get(market.uniqueKey)!;
    console.log(
      `Processing market ${cap.label} (${market.uniqueKey.slice(0, 10)}...) with cap ${(
        cap.capApr * 100
      ).toFixed(2)}%`
    );

    await upsertMarketMetadata(market, cap.capApr, cap.chainId);
    ...
    if (ratePoints.length === 0) {
      const fallbackApy = market.state?.netBorrowApy ?? market.state?.borrowApy ?? 0;
      ratePoints.push({
        timestamp: dayStart,
        apy: fallbackApy,
      });
    ...
    for (const borrower of borrowers) {
      try {
        const result = await processBorrower({
          borrower,
          market,
          capApr: cap.capApr,
          chainId: cap.chainId,
        ...
        reimbursementsCreated += result.reimbursementCreated;
      } catch (error) {
  ...
  return {
    ...
    marketsProcessed: targets.length,
    borrowersProcessed,
